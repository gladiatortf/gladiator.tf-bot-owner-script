...
const Settings = {
  ...
  form: {
    render: () => {
      ...
      (botAmount > 0 ? $select : $(`<span>You dont have any bots</span>`)).insertAfter(
        $bots.find('label')
      );
      $parent.append($bots);
      $bots.after($reload);
...
function backpackUserscript(pathname) {
  ...
  function bulkAdd(itemNames) {
    ...
    const error = (msg) => {
      const modal = ['Error', msg];
      Modal.render(...modal);
    };

    GM_xmlhttpRequest({
      method: 'POST',
      url: `https://${GLAD_DOMAIN}/api/bots/${
      ...
      onload: function (data) {
        try {
          ...
          const botName = Object.entries(Settings.data.bots).filter(
          ...
          let msg =
            failedAdds.length === 0
              ...
              : `Some items failed to be added (${results.length - failedAdds.length}/${
                  results.length
                } Successful):<br> ${failedAdds.join('<br>')}`;
  ...
  function effect() {
    appendAddButtons([['Add All Priced', '#unusual-pricelist']], '.input-group:first');
  }
    ...
    const loadAll = (addKS) => {
      return new Promise(async (resolve) => {
        if ($('#pricelist').is('table')) {
          ...
          let items = [];
          let iterator = 1;
          const last = $('#pricelist-pagination-container a:contains("Last")').attr(
            'href'
          );
  ...
  function settings() {
    ...
    const $settings = $(
      `<li class="li-gladiator-options"><a>${svg.options} Bot Settings</a> </li>`
    );
    $settings.on('click', () =>
          ...
          if (popover.find(`a[href^='https://${GLAD_DOMAIN}']`).length == 0) {
            popover.append(
              '<a class="btn btn-default btn-xs" href="' +
                `https://${GLAD_DOMAIN}/manage/${
                  Settings.data.manageContext
                }/item/${encodeURIComponent($($(self)[0]).data('original-title'))}/add` +
  ...
  function reloadManageLink() {
    ...
    $('.gladiator-context').each(function () {
      $(this).attr(
        'href',
        `https://${GLAD_DOMAIN}/manage/${manageContext}${$(this).data('postfix')}`
      );
    });
